...
FEATURES = {
    ...
    "states.left_joint.position": {
        "dtype": "float32",
        "shape": (7,),
        "names": ["left_joint_0", "left_joint_1", "left_joint_2", "left_joint_3", "left_joint_4", "left_joint_5", "left_joint_6"],
    },
    ...
    "states.right_joint.position": {
        "dtype": "float32",
        "shape": (7,),
        "names": ["right_joint_0", "right_joint_1", "right_joint_2", "right_joint_3", "right_joint_4", "right_joint_5","right_joint_6"],
    },
...
class Genie1Dataset(LeRobotDataset):
    def __init__(
        self,
        repo_id: str,
...
def load_lmdb_data(episode_path: Path, sava_path: Path, fps_factor: int, target_fps: int) -> Optional[Dict]:
    def load_image(txn, key):
        ...
        return image
    # set_trace()
    if episode_path.name.startswith("00"):
        # for simbox
        left_armbase_to_robot_pose = np.array([[ 8.77583782e-01, -2.06711634e-07,  4.79423306e-01, 2.77224123e-01],
                                                [ 3.04746970e-07,  1.00000000e+00, -1.26671697e-07, -5.41924611e-08],
                                                [-4.79423306e-01,  2.57267827e-07,  8.77583782e-01, 1.29616246e+00],
                                                [ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00, 1.00000000e+00]])
    elif episode_path.name.startswith("2025"):
        # for dataengine
        left_armbase_to_robot_pose = np.array([[ 8.08028872e-01, -3.27251376e-08,  5.89142888e-01, 3.10688674e-01],
                                                [ 2.22104410e-07,  1.00000000e+00, -2.49076482e-07, -8.51402262e-08],
                                                [-5.89142888e-01,  3.32112222e-07,  8.08028872e-01, 1.21495271e+00],
                                                [ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00, 1.00000000e+00]])
    right_armbase_to_robot_pose = left_armbase_to_robot_pose
    ...
    tcp2ee_pose[2, 3] = 0.22

    # model = pin.buildModelFromUrdf("../assets/R5a/R5a.urdf")
            ...
            for compute_qpos_key in compute_qpos_keys:
                ...
                for each_compute_qpos in compute_qpos:
                    if "left" in compute_qpos_key:
                        model = pin.buildModelFromUrdf("../assets/G1_120s/left_arm_parallel.urdf")
                    elif "right" in compute_qpos_key:
                        model = pin.buildModelFromUrdf("../assets/G1_120s/right_arm_parallel.urdf")
                    data = model.createData()
                    ...
                    fid_a = model.getFrameId("arm_base_link")
                    if "left" in compute_qpos_key:
                        fid_b = model.getFrameId("arm_l_end_link")
                    elif "right" in compute_qpos_key:
                        fid_b = model.getFrameId("arm_r_end_link")
...
def save_as_lerobot_dataset(task: tuple[Path, Path], repo_id, num_threads, debug, origin_fps=30,  target_fps=30, num_demos=None, robot_type="piper", delete_downsampled_videos=True):
