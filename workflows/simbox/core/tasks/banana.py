    ...
    def _set_obstaclecontact_view(self, cfg):
        ...
        for robot_name, robot in self.robots.items():
            ...
            for obstacle_name in obstacle_names:
                if os.environ.get("INTERNDATA_ISAAC5_COMPAT") == "1":
                    # Isaac 5 compatibility collapses obstacle links into
                    # the registered aggregate root rigid body.
                    source_expr = self.objects[obstacle_name].prim_path
                else:
    ...
    def _fix_obstacle_physics_hierarchy(self):
        ...
        for obj_name, obj in self.objects.items():
            ...
            if child_rigid_bodies:
                # The referenced MANO USD also contains joints between its
                # original child bodies. Once those child bodies are
                # collapsed into the aggregate root, retaining the joints
                # makes PhysX bind both endpoints to the same body.
                joint_prims = [
                    child for child in Usd.PrimRange(prim)
                    if child != prim and child.IsA(UsdPhysics.Joint)
                ]
                for joint in joint_prims:
                    # Referenced joint prims are ancestral, so RemovePrim
                    # cannot mask them in the composed stage. Deactivate
                    # each joint explicitly before PhysX stage parsing.
                    joint.SetActive(False)
                if joint_prims:
                    print(
                        f"[physics_compat] removed {len(joint_prims)} nested "
                        f"obstacle joints under {prim.GetPath()}"
                    )
                # RigidObject applies RigidBodyAPI to the configured root.
                # Keep that aggregate body (the wrapper's view expression)
                # and remove nested bodies to avoid invalid PhysX topology.
                for child in child_rigid_bodies:
            ...
            # Isaac Sim 5 requires every object registered as a RigidObject to
            # resolve to an actual PhysX rigid body.  The MANO hand asset used
            # by the safety overlay only authors CollisionAPI on its mesh
            # children, so a RigidPrimView for ``.../obstacle_1/mano`` would
            # otherwise match zero bodies and fail in ``max_shapes`` during
            # World.reset().  Promote the obstacle root to a kinematic rigid
            # body when the asset has no child bodies.  This preserves the
            # authored geometry while allowing pose/velocity queries and
            # prevents the surrogate hand from falling under gravity.
            if not child_rigid_bodies and not prim.HasAPI(UsdPhysics.RigidBodyAPI):
                # Some referenced hand assets put CollisionAPI on nested
                # descendants, so checking only direct collision children is
                # insufficient.  Applying RigidBodyAPI to this configured
                # object root guarantees that its RigidPrimView expression
                # resolves to a body in Isaac Sim 5.
                ...
                collision_count = sum(
                    1
                    for child in Usd.PrimRange(prim)
                    if child != prim and child.HasAPI(UsdPhysics.CollisionAPI)
                )

                print(
                    f"[physics_compat] promoted obstacle root to kinematic "
                    f"rigid body: {prim.GetPath()} "
